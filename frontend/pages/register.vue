<template>
  <div class="min-h-screen bg-gradient-soft flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    ...
    <div class="relative max-w-md w-full space-y-8">
      ...
      <!-- 已登入用戶切換提示 -->
      <AuthSwitchPrompt
        v-if="showSwitchPrompt && !isCheckingAuth"
        page-type="register"
        :title="switchPromptTitle"
        :description="switchPromptDescription"
        :user-display-name="userDisplayName"
        @continue="continueWithCurrentAccount"
        @switch="switchToOtherAccount"
      />

      <!-- Loading 狀態 -->
      <div
        v-if="isCheckingAuth"
        class="card-elevated p-8 animate-slide-up flex items-center justify-center"
      >
        <div class="flex items-center space-x-3">
          ...
          <span class="text-gray-600">檢查登入狀態...</span>
        </div>
      </div>

      <!-- Register Form Card -->
      <div
        v-if="!isCheckingAuth && !showSwitchPrompt"
...
} = useAuthRedirect({ pageType: 'register' })
