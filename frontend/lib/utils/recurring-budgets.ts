...
export function calculatePeriodDates(frequency: TemplateFrequency, period: string): {
  ...
  if (frequency === 'monthly') {
    ...
    const month = Number.parseInt(monthStr) - 1 // Date 的月份從 0 開始
  ...
  if (frequency === 'quarterly') {
    ...
    const year = Number.parseInt(yearStr)
    const quarter = Number.parseInt(quarterStr) // 1-4
...
export function getNextPeriod(frequency: TemplateFrequency, currentPeriod: string): string {
  ...
  if (frequency === 'yearly') {
    const year = Number.parseInt(currentPeriod)
    return `${year + 1}`
  }
...
export function formatPeriodDisplay(frequency: TemplateFrequency, period: string): string {
  ...
  if (frequency === 'quarterly') {
    ...
    const quarterNum = Number.parseInt(quarterStr, 10) as 1 | 2 | 3 | 4
    return `${yearStr}年${quarterMap[quarterNum]}`
  }
